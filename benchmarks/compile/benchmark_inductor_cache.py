...
def print_model_results(model_name: str, results: Dict[str, float], run_type: str = ""):
    ...
    print(f"{prefix}Total Time:       {format_time(results['load_time'] + results['inference_time'])}")
...
def print_summary(all_results: Dict[str, Dict[str, any]]):
    ...
    for model_name in sorted(all_results.keys()):
        ...
        if 'cache_miss' in results and 'cache_hit' in results:
            ...
            model_display = model_name if len(model_name) <= 45 else model_name[:42] + "..."
            
            print(f"{model_display:<45} {miss_load:<12} {hit_load:<12} {load_speedup:<10} {miss_total:<12} {hit_total:<12} {total_speedup:<10}")
        else:
            ...
            model_display = model_name if len(model_name) <= 45 else model_name[:42] + "..."
            print(f"{model_display:<45} {load_time:<12} {'N/A':<12} {'N/A':<10} {total_time:<12} {'N/A':<12} {'N/A':<10}")
...
def main():
        ...
        for model_spec in args.models:
            ...
            try:
                ...
                if 0 <= idx < len(DEFAULT_MODELS):
                    ...
                    print(f"ERROR: Model index {idx} out of range (0-{len(DEFAULT_MODELS)-1})")
                    return
            except ValueError:
    ...
    for model_config in models_to_run:
        ...
        try:
            ...
            load_speedup = cache_miss_results['load_time'] / cache_hit_results['load_time']
            total_speedup = (cache_miss_results['load_time'] + cache_miss_results['inference_time']) / \
